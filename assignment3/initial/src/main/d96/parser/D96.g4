...
array_type: ARRAY LSB (data_types) COMMA NATURAL  RSB; //var a: Array[Int, 5];
class_type: ID;
...
expr_8 : expr_8 DOT ID (LP (expr (COMMA expr)*)? RP)?   | expr_9; 	//binary, infix, left-associate
expr_9 : expr_10 STATIC_GLOBAL_DOT DOLLAR_ID (LP (expr (COMMA expr)*)? RP)? | expr_10; //binary, infix, left-associate
expr_10: NEW expr_10  | expr_11 ; 	//unary, prefix, right-associate
expr_11: literals    | SELF  |((ID|DOLLAR_ID)index_id) | indexed_array  |  NULL | class_new_object | identifier  |(LP expr RP); 
// | method_invocate_expression
...
NATURAL: DEC_ 		{self.text=self.text.replace("_","")}
	| OCT_		{self.text=self.text.replace("_","")}
	| HEX_ 	{self.text=self.text.replace("_","")}
	| BIN_ 		{self.text=self.text.replace("_","")}
	; 
BIN_      	: [0][bB] ( ([1] ('_'[01] | [01])*) ); 
DEC_		: [1-9][0-9]* ('_' [0-9]+)*;
OCT_       	: [0] ( [1-7] ( [0-7] | ('_'[0-7]))* );
HEX_ 		: [0][xX] ( [1-9A-F]  ('_'[0-9A-F] | [0-9A-F])* );

fragment DECIMAL     : [0] |  ([1-9] ( [0-9] | '_'[0-9] )* ) ;
fragment BINARY      : [0][bB] ( ([1] ('_'[01] | [01])*) | [0] ); 
fragment OCTAL       : [0] ( [1-7] ( [0-7] | ('_'[0-7]))* | [0]);
fragment HEXADECIMAL : [0][xX] ( [1-9A-F]  ('_'[0-9A-F] | [0-9A-F])*  | [0]);
